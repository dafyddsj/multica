...
| AgentMail | `MULTICA_AGENTMAIL_SECRET_KEY` | base64로 인코딩한 32바이트 암호화 키. 없으면 이메일 탭이 숨겨집니다 |
| AgentMail | `MULTICA_AGENTMAIL_ORG_KEY` | hosted 조직 키. 프로세스 환경에만 둡니다. `AGENTMAIL_ORG_API_KEY`도 허용 |
| AgentMail | `MULTICA_AGENTMAIL_API_BASE` | 선택 REST 루트. `/v0` 포함(기본값 `https://api.agentmail.to/v0`) |
...
설정 단계는 [GitHub 연동](/github-integration#자체-호스팅-설정), [Feishu Bot](/lark-bot-integration#자체-호스팅-설정), [Slack Bot](/slack-bot-integration#자체-호스팅-설정), [Telegram Bot](/telegram-bot-integration#관리-및-자체-호스팅), [AgentMail](/agentmail-integration)을 참고하세요.
...
각 AI 코딩 도구는 `MULTICA_<PROVIDER>_PATH`로 명령 경로를 덮어쓸 수 있으며 모델 재정의를 지원하는 도구는 `MULTICA_<PROVIDER>_MODEL`도 사용할 수 있습니다. QwenPaw와 MiniMax Code에는 모델 변수가 없고 MiniMax Code의 경로 변수는 `MULTICA_MCODE_PATH`입니다. 자세한 내용은 [AI 코딩 도구 비교](/providers#모델-출처)를 참고하세요. DeepSeek Harness는 `MULTICA_DSH_PATH`와 `MULTICA_DSH_MODEL`을 지원합니다(값은 dsh 모델 카탈로그의 모델 ID, 예: `deepseek-official/deepseek-chat`). ZeroClaw는 `MULTICA_ZEROCLAW_PATH`를 지원하지만 모델 변수는 없습니다. 모델은 ZeroClaw의 에이전트 설정에서 관리합니다. Amp는 `MULTICA_AMP_PATH`와 `AMP_API_KEY` 전달을 지원합니다. 에이전트 모델 필드는 `--mode`로 전달되며 `MULTICA_AMP_MODEL`은 없습니다. Devin은 `MULTICA_DEVIN_PATH`를 지원합니다. `MULTICA_DEVIN_MODEL`은 없습니다. 선택 상자는 `devin models list --format json` 결과를 나열합니다. Goose는 `MULTICA_GOOSE_PATH`를 지원합니다. `MULTICA_GOOSE_MODEL`은 없습니다. 에이전트 모델이 설정되면 `--model`로 전달됩니다. 컴퓨터 전체 기본 인수 `MULTICA_<PROVIDER>_ARGS`는 현재 Claude Code, Codex, CodeBuddy, Qwen Code, QwenPaw, Amp, Devin, Goose 여덟 도구를 지원합니다. 해당 변수는 `MULTICA_CLAUDE_ARGS`, `MULTICA_CODEX_ARGS`, `MULTICA_CODEBUDDY_ARGS`, `MULTICA_QWEN_ARGS`, `MULTICA_QWENPAW_ARGS`, `MULTICA_AMP_ARGS`, `MULTICA_DEVIN_ARGS`, `MULTICA_GOOSE_ARGS`입니다. 예시:
...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `MULTICA_FEATURE_FLAGS_FILE` | 비어 있음 | 배포 플래그 YAML 규칙 파일 |
| `FF_<FLAG_KEY>` | 미설정 | 플래그 단위 재정의. YAML보다 우선. `true` / `on` / `1`이면 켜짐 |

Memory v1은 `FF_MEMORY_V1=true`(또는 YAML의 `memory_v1`)와 워크스페이스 Labs 토글이 모두 켜져야 동작합니다. [메모리](/memory)를 참고하세요.
