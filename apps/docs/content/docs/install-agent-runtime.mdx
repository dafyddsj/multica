...
| Devin | `devin` | [Devin CLI](https://docs.devin.ai/cli) |
...
For Amp, sign in with `amp login` or set `AMP_API_KEY`. Amp picks its own model; Multica does not send `--model`.

For Devin, sign in with `devin auth login`. v1 is the local CLI (`devin --print`), not Devin Cloud. Multica lists models from `devin models list --format json` on the daemon host. Login is required. An empty list still allows manual entry.
For Goose, run `goose configure` and set `GOOSE_PROVIDER`, `GOOSE_MODEL`, and the vendor API key for that provider. Multica does not act as a Goose identity provider. Type a model id in Multica to send `--model`. Leave the field empty to use Goose's configured default. Type a Goose provider id (`openrouter`, `ollama`, …) to send `--provider`, or leave it empty to use `GOOSE_PROVIDER`.
