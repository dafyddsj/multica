...
For Goose, run `goose configure` and set `GOOSE_PROVIDER`, `GOOSE_MODEL`, and the vendor API key for that provider. Multica does not act as a Goose identity provider. The model picker stays on "Managed by runtime" until Goose can list models without a signed-in provider.
