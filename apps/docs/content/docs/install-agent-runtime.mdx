...
For Goose, run `goose configure` and set `GOOSE_PROVIDER`, `GOOSE_MODEL`, and the vendor API key for that provider. Multica does not act as a Goose identity provider. Type a model id in Multica to send `--model`. Leave the field empty to use Goose's configured default.
