...
For Devin, sign in with `devin auth login`. v1 is the local CLI (`devin --print`), not Devin Cloud. Multica does not list Devin models until a logged-in `devin models list` canary.
