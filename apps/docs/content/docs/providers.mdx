...
QwenPaw, MiniMax Code, and Goose are exceptions: they do not accept a model chosen from Multica, so the picker is inert and shows "Managed by runtime". Choose the model in the tool's own configuration instead. An empty model list is expected there and is not a sign that the runtime is offline.
...
Antigravity, GitHub Copilot CLI, DevEco Code, Goose, and Pi do not read the MCP configuration on an agent yet, so the field is not shown when creating agents for them.
