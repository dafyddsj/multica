...
"Multica-managed MCP" means you can define MCP servers in the agent configuration and Multica passes them to the tool before a run. It does not mean the tool has no other way to configure MCP. Amp's `--mcp-config` merges with existing settings; Multica also points `--settings-file` at a per-run file with empty `amp.mcpServers` so global Amp MCP is not inherited. Servers from the Amp account or a workspace `.amp/settings.json` may still load.
...
Amp's picker lists Amp modes (`low`, `medium`, `high`, `ultra`). A mode chooses the model, system prompt, and tools together. Leave it empty to use Amp's default (`medium`). Plugin modes can be typed as a custom value.
...
Devin lists models from the live CLI on the daemon host (`devin models list --format json`). The picker is supported. The list is empty until you are logged in; you can still type a model id.

Goose's empty catalog is expected. Type a Goose model id, or leave the field empty to use `GOOSE_MODEL`. Type a Goose provider id (`openrouter`, `ollama`, …) or leave it empty to use `GOOSE_PROVIDER`.
...
Multica stores the session identifier the tool returns. The next time work continues on the same issue, the runtime tries to resume that session; when it cannot, it creates a new session and keeps going. Pi stores a local session file path, so it depends more on the original computer and file. Goose 1.48.0 stream-json does not emit a session id. Multica persists one when a later Goose release includes a parseable `session_id`. Until then, each Goose run starts a new session.

Retry and session-resumption rules are in [Tasks](/tasks#manual-retry).

## MCP configuration

Antigravity, GitHub Copilot CLI, DevEco Code, Devin, Goose, and Pi do not read the MCP configuration on an agent yet, so the field is not shown when creating agents for them. Devin v1 is the local CLI (`devin --print`), not Devin Cloud.
