...
Antigravity, GitHub Copilot CLI, DevEco Code, Devin, and Pi do not read the MCP configuration on an agent yet, so the field is not shown when creating agents for them. Devin v1 is the local CLI (`devin --print`), not Devin Cloud.
