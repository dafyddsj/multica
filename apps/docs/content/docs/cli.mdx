...
Memory notes are off until the `memory_v1` flag and the workspace Labs toggle are both on. See [Memory](/memory).

```bash
multica memory add --scope issue --owner-id <issue-id> --body "Prefer rebase"
multica memory list --scope issue --owner-id <issue-id>
multica memory recall --issue-id <issue-id>
...
| `initiative` | `list/get/create/update/delete` | Manage initiatives | `create`: `--title` (required), `--description`, `--status`, `--icon`, `--lead`, `--start-date`, `--due-date` |
| | `status <id> <status>` | Change initiative status | |
| `project` | `list/get/create/update/delete` | Manage projects | `create` / `update`: `--initiative` |
