...
- **Everyday noun** — the word a user would say out loud for it. **Translate fully**, whether or not it is a database entity: `issue` → 任务, `workspace` → 工作区, `project` → 项目, `initiative` → 专项.
...
- **`initiative` → "专项"**: settled. Linear uses 专项 in Chinese products. 专项 is the locked term. Japanese is イニシアチブ. Korean is 이니셔티브.
