...
| AgentMail | `MULTICA_AGENTMAIL_SECRET_KEY` | Base64-encoded 32-byte encryption key; without it the Email tab stays hidden |
| AgentMail | `MULTICA_AGENTMAIL_ORG_KEY` | Hosted organization key in process env only; `AGENTMAIL_ORG_API_KEY` is an alias |
| AgentMail | `MULTICA_AGENTMAIL_API_BASE` | Optional REST root, including `/v0` (default `https://api.agentmail.to/v0`) |
...
For setup steps, see [GitHub integration](/github-integration#self-hosting-setup), [Lark bot](/lark-bot-integration#self-hosting-setup), [Slack bot](/slack-bot-integration#self-hosting-setup), [Telegram bot](/telegram-bot-integration#self-hosting-setup), and [AgentMail](/agentmail-integration).
...
Each AI coding tool accepts `MULTICA_<PROVIDER>_PATH` and `MULTICA_<PROVIDER>_MODEL` to override the command path and default model. QwenPaw and MiniMax Code have no model variable because Multica never sends them a model; MiniMax Code's path variable is `MULTICA_MCODE_PATH` — see [AI coding tools comparison](/providers#model-sources). DeepSeek Harness supports `MULTICA_DSH_PATH` and `MULTICA_DSH_MODEL` (a model id from the dsh catalog, e.g. `deepseek-official/deepseek-chat`). Dim supports `MULTICA_DIM_PATH` and `MULTICA_DIM_MODEL` (a model id from the dim catalog). ZeroClaw supports `MULTICA_ZEROCLAW_PATH` but has no model variable; its agent profile owns the model selection. Amp supports `MULTICA_AMP_PATH` and `AMP_API_KEY` passthrough. The agent model field is sent as `--mode`; there is no `MULTICA_AMP_MODEL`. Devin supports `MULTICA_DEVIN_PATH`. There is no `MULTICA_DEVIN_MODEL`. The picker lists `devin models list --format json`. Goose supports `MULTICA_GOOSE_PATH`. There is no `MULTICA_GOOSE_MODEL`. Per-agent model is sent as `--model` when set. Machine-wide default arguments via `MULTICA_<PROVIDER>_ARGS` are currently supported for eight tools: Claude Code, Codex, CodeBuddy, Qwen Code, QwenPaw, Amp, Devin, and Goose. The variables are `MULTICA_CLAUDE_ARGS`, `MULTICA_CODEX_ARGS`, `MULTICA_CODEBUDDY_ARGS`, `MULTICA_QWEN_ARGS`, `MULTICA_QWENPAW_ARGS`, `MULTICA_AMP_ARGS`, `MULTICA_DEVIN_ARGS`, and `MULTICA_GOOSE_ARGS`. For example:
...
| Variable | Default | Description |
|---|---|---|
| `MULTICA_FEATURE_FLAGS_FILE` | empty | YAML rule file for deployment flags |
| `FF_<FLAG_KEY>` | unset | Per-flag override. Beats the YAML file. `true` / `on` / `1` enables |

Memory v1 is off until both `FF_MEMORY_V1=true` (or a YAML `memory_v1` rule) and the workspace Labs toggle are on. See [Memory](/memory).
