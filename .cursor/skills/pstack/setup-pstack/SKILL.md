...
Write `~/.cursor/rules/pstack-models.mdc` and, when this repository has `.cursor/rules/pstack-models.mdc`, the same file in the repo. Cloud Agent VMs cannot read `~/.cursor/rules`, so the repo copy is what those runs see. The skills read either file and fall back to their inline defaults when a line is absent, so this is an override layer, not a requirement.
...
The default role-to-model mapping is the rule shape shown in step 5 below. If `.cursor/rules/pstack-models.mdc` or `~/.cursor/rules/pstack-models.mdc` already exists, read it (prefer the repo copy when both exist) and treat its values as the current choices. Otherwise start from those defaults.
...
Write `~/.cursor/rules/pstack-models.mdc` with `alwaysApply: true` and one line per role, using the same labels poteto-mode uses. Overwrite the whole file so re-runs stay idempotent. When `.cursor/rules/` exists in the current repository, write the same contents to `.cursor/rules/pstack-models.mdc` so Cloud Agents keep the seats. Shape:
