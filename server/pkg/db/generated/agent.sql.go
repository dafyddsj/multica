...
func (q *Queries) AcknowledgeExhaustedDelegatedFailureRecovery(ctx context.Context, arg AcknowledgeExhaustedDelegatedFailureRecoveryParams) (AgentTaskQueue, error) {
	...
	err := row.Scan(
		...
		&i.ModelOverride,
		&i.BudgetProjectID,
		&i.BudgetInitiativeID,
		&i.BudgetOriginSquadID,
	)
	return i, err
}
...
RETURNING task.id, task.agent_id, task.issue_id, task.status, task.priority, task.dispatched_at, task.started_at, task.completed_at, task.result, task.error, task.created_at, task.context, task.runtime_id, task.session_id, task.work_dir, task.trigger_comment_id, task.chat_session_id, task.autopilot_run_id, task.attempt, task.max_attempts, task.parent_task_id, task.failure_reason, task.trigger_summary, task.force_fresh_session, task.is_leader_task, task.wait_reason, task.initiator_user_id, task.handoff_note, task.prepare_lease_expires_at, task.squad_id, task.runtime_mcp_overlay, task.escalation_for_task_id, task.fire_at, task.originator_user_id, task.runtime_connected_apps, task.coalesced_comment_ids, task.delivered_comment_ids, task.chat_input_task_id, task.chat_finalize_deferred_at, task.originator_source, task.delegated_from_task_id, task.retry_of_task_id, task.rerun_of_task_id, task.rule_version_id, task.trigger_evidence_kind, task.trigger_evidence_ref_id, task.accountable_user_id, task.session_rollout_missing, task.retired_session_id, task.quick_actions_disabled, task.regenerate_quick_actions_for, task.branch_name, task.durable_work_dir, task.channel_context_revision, task.execution_lane, task.model_override, task.budget_project_id, task.budget_initiative_id, task.budget_origin_squad_id
`
...
func (q *Queries) ClearAgentCoAuthoredByEmail(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentCoAuthoredByEmail, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
func (q *Queries) ClearAgentFailoverModel(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentFailoverModel, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
func (q *Queries) ClearAgentFailoverRuntimeID(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentFailoverRuntimeID, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
func (q *Queries) ClearAgentFailoverServiceTier(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentFailoverServiceTier, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
func (q *Queries) ClearAgentFailoverThinkingLevel(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentFailoverThinkingLevel, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
func (q *Queries) ClearAgentLightweightModel(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentLightweightModel, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
func (q *Queries) ClearAgentLightweightThinkingLevel(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentLightweightThinkingLevel, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
type CreateAgentParams struct {
	...
	StartLightweight         interface{} `json:"start_lightweight"`
...
func (q *Queries) CreateAgent(ctx context.Context, arg CreateAgentParams) (Agent, error) {
	row := q.db.QueryRow(ctx, createAgent,
		...
		arg.PermissionMode,
		arg.LightweightModel,
		arg.LightweightThinkingLevel,
		arg.StartLightweight,
		arg.FailoverRuntimeID,
		arg.FailoverModel,
		arg.FailoverThinkingLevel,
		arg.FailoverServiceTier,
	)
	var i Agent
	err := row.Scan(
		&i.ID,
...
type CreateAgentTaskParams struct {
	...
	TriggerEvidenceKind  pgtype.Text   `json:"trigger_evidence_kind"`
	TriggerEvidenceRefID pgtype.UUID   `json:"trigger_evidence_ref_id"`
	ExecutionLane        pgtype.Text   `json:"execution_lane"`
	ModelOverride        pgtype.Text   `json:"model_override"`
	BudgetProjectID      pgtype.UUID   `json:"budget_project_id"`
	BudgetInitiativeID   pgtype.UUID   `json:"budget_initiative_id"`
	BudgetOriginSquadID  pgtype.UUID   `json:"budget_origin_squad_id"`
...
func (q *Queries) CreateAgentTask(ctx context.Context, arg CreateAgentTaskParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, createAgentTask,
		...
		arg.ModelOverride,
		arg.BudgetProjectID,
		arg.BudgetInitiativeID,
		arg.BudgetOriginSquadID,
		arg.ID,
	)
	var i AgentTaskQueue
	err := row.Scan(
...
SELECT
    p.agent_id,
    COALESCE($2, p.runtime_id) AS runtime_id,
    p.issue_id,
    p.chat_session_id,
    p.autopilot_run_id,
    CASE WHEN $3::timestamptz IS NOT NULL THEN 'deferred' ELSE 'queued' END AS status,
    ...
    COALESCE($5::int, p.max_attempts) AS max_attempts,
    ...
    $6 AS runtime_mcp_overlay,
    $7 AS runtime_connected_apps,
    p.originator_source,
    ...
    COALESCE($8::text, p.execution_lane) AS execution_lane,
    COALESCE($9, p.model_override) AS model_override,
    p.budget_project_id,
    p.budget_initiative_id,
    p.budget_origin_squad_id,
    -- Named new_task_id, not id: $1 above is the PARENT task's id.
    COALESCE($10::uuid, gen_random_uuid()) AS id
FROM agent_task_queue p
WHERE p.id = $1
  AND lock_task_owner_rows(p.agent_id, p.issue_id, COALESCE($2, p.runtime_id))
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, squad_id, runtime_mcp_overlay, escalation_for_task_id, fire_at, originator_user_id, runtime_connected_apps, coalesced_comment_ids, delivered_comment_ids, chat_input_task_id, chat_finalize_deferred_at, originator_source, delegated_from_task_id, retry_of_task_id, rerun_of_task_id, rule_version_id, trigger_evidence_kind, trigger_evidence_ref_id, accountable_user_id, session_rollout_missing, retired_session_id, quick_actions_disabled, regenerate_quick_actions_for, branch_name, durable_work_dir, channel_context_revision, execution_lane, model_override, budget_project_id, budget_initiative_id, budget_origin_squad_id FROM agent_task_queue
WHERE id = $1
`
...
UPDATE agent SET
    ...
    co_authored_by_email = COALESCE($21, co_authored_by_email),
    lightweight_model = COALESCE($22, lightweight_model),
    lightweight_thinking_level = COALESCE($23, lightweight_thinking_level),
    start_lightweight = COALESCE($24, start_lightweight),
    failover_runtime_id = COALESCE($25, failover_runtime_id),
    failover_model = COALESCE($26, failover_model),
    failover_thinking_level = COALESCE($27, failover_thinking_level),
...
type UpdateAgentParams struct {
	...
	LightweightModel         pgtype.Text `json:"lightweight_model"`
	LightweightThinkingLevel pgtype.Text `json:"lightweight_thinking_level"`
	StartLightweight         pgtype.Bool `json:"start_lightweight"`
	FailoverRuntimeID        pgtype.UUID `json:"failover_runtime_id"`
	FailoverModel            pgtype.Text `json:"failover_model"`
	FailoverThinkingLevel    pgtype.Text `json:"failover_thinking_level"`
	FailoverServiceTier      pgtype.Text `json:"failover_service_tier"`
