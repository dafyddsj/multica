...
| Amp daemon-owned argv | `pkg/agent/amp.go` `ampBlockedArgs` / `filterAmpThreadSequences` / `buildAmpArgs` | `--execute`, stream-json flags, `--dangerously-allow-all`, `--mcp-config`, `--resume`, `--continue`, the `threads continue <T-uuid>` sequence, `--no-tui`, `--executor`, `-p`, and `--output-format` are stripped from `custom_args` / `MULTICA_AMP_ARGS`. Resume is only `threads continue` with a parsed `T-<uuid>`. |
