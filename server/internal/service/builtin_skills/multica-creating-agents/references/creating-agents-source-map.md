...
| Amp daemon-owned argv | `pkg/agent/amp.go` `ampBlockedArgs` / `filterAmpThreadSequences` / `buildAmpArgs` | `--execute`, stream-json flags, `--dangerously-allow-all`, `--no-archive-after-execute`, `--unarchive`, `--mcp-config`, `--settings-file`, `--mode` / `-m`, `--resume`, `--continue`, the `threads continue <T-uuid>` sequence, `--no-tui`, `--executor`, `-p`, and `--output-format` are stripped from `custom_args` / `MULTICA_AMP_ARGS`. `agent.model` is passed as `--mode`. Resume is `threads archive --unarchive` then `threads continue` with a parsed `T-<uuid>`. |
| Devin daemon-owned argv | `pkg/agent/devin.go` `devinBlockedArgs` / `filterDevinCommands` / `buildDevinArgs` | `--print` / `-p`, `--prompt-file`, `--permission-mode`, `--respect-workspace-trust`, `--model`, `-c` / `--continue`, `-r` / `--resume`, `--sandbox`, and `acp` / `cloud` / `ssh` / `desktop` are stripped from `custom_args` / `MULTICA_DEVIN_ARGS`. Prompt text is a 0600 `--prompt-file`. Resume is `--resume <parsed-id>` only. |
| Goose daemon-owned argv | `pkg/agent/goose.go` `gooseBlockedArgs` / `buildGooseArgs` | `-i` / `--instructions`, `-t` / `--text`, `--output-format`, `--resume`, `-r`, `--session-id`, `--no-session`, `-s` / `--interactive`, `--recipe`, `--acp`, `acp`, `serve`, and `--provider` are stripped from `custom_args` / `MULTICA_GOOSE_ARGS`. Fresh argv is `goose run -i - --output-format stream-json`. Resume is `--resume --session-id <YYYYMMDD_<id>>` together. The prompt is plaintext on stdin. When set, `agent.model` is passed as `--model`. `runtime_config.goose_provider` is `--provider`. |
