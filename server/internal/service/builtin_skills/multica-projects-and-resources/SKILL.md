...
A project groups work and carries durable resources. A resource is not just display metadata; it is context later injected into task briefs and `.multica/project/resources.json`. Status keys default to `planned`, `in_progress`, `paused`, `completed`, `cancelled`; a workspace can add custom keys under Settings → Statuses, and `multica project status` accepts those keys.
