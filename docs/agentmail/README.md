...
- Grant is create-new (username + domain, including custom domains) or link-existing
- Turn-off asks keep vs delete; delete requires a second confirm
...
Inbound webhooks, a partial unique index on `remote_inbox_id`, pause (keep the
address, drop only the overlay), mobile, and landing-page copy. See `NEXT.md`.
Create-vs-link and keep-vs-delete already shipped. Do not reopen them.

## Names

| In the product | Meaning |
| --- | --- |
| Email | The Settings tab and the first-class agent tab (sibling of Work) |
