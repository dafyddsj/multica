...
The Budgets tab of `/{slug}/usage` can create and edit a budget and show a server-priced bar.

## Changes

- `packages/views/dashboard/components/budgets-tab.tsx` and `budget-form-dialog.tsx`.
- Mount as its own Analytics tab between Usage and Errors. `?tab=budgets` opens it. Spend filters stay off this tab because caps are UTC-month, not the Usage time range.
...
- Owner and admin see Bypass limits on project and initiative rows. Window defaults to now through end of the current UTC month. Optional reason. The chip reads Limits waived until {date}. Agent and squad rows have no waiver control.
...
**Runtime.** In the browser, create a $50 project budget with soften at 80 and pause. The tab shows the backfilled bar. Edit the limit. As owner, waive the project for the rest of the month. Delete the budget. Check desktop only if you have it. Mobile is out.
