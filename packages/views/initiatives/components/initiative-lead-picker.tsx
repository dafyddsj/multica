...
import type { Initiative, UpdateInitiativeRequest } from "@multica/core/types";
import { useT } from "../../i18n";
import { matchesPinyin } from "../../editor/extensions/pinyin-match";
import { ActorAvatar } from "../../common/actor-avatar";

export function InitiativeLeadPicker({
  initiative,
  handleUpdate,
  renderTrigger,
  align = "start",
  ...
  const filteredMembers = members.filter(
    (m) => m.name.toLowerCase().includes(leadQuery) || matchesPinyin(m.name, leadQuery),
  );
  ...
  const leadId = initiative.lead_id;
  const leadType = initiative.lead_type;
