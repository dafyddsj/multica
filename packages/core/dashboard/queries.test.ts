...
describe("dashboard range placeholders", () => {
  it.each(optionBuilders)(
    ...
    (buildOptions) => {
      ...
      const previousOptions = buildOptions("ws-1", 30, null, null, "UTC");
      const nextOptions = buildOptions("ws-1", 7, null, null, "UTC");
  ...
  it("does not carry placeholder data across workspace, project, initiative, or timezone scopes", () => {
    ...
    const previousScopes = [
      ...
      dashboardUsageDailyOptions("ws-1", 30, "project-2", "initiative-1", "Asia/Shanghai"),
      dashboardUsageDailyOptions("ws-1", 30, "project-1", "initiative-2", "Asia/Shanghai"),
      dashboardUsageDailyOptions("ws-1", 30, "project-1", "initiative-1", "UTC"),
    ];
  ...
  it("keeps the initial loading state honest when there is no previous query", () => {
    const options = dashboardUsageDailyOptions("ws-1", 30, null, null, "UTC");
