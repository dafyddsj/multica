...
export function toStoredAgentDraft(
  ...
  return {
    ...
    lightweight_model: draft.lightweightModel,
    lightweight_thinking_level: draft.lightweightThinkingLevel,
    start_lightweight: draft.startLightweight,
    failover_runtime_id: draft.failoverRuntimeId,
    failover_model: draft.failoverModel,
    failover_thinking_level: draft.failoverThinkingLevel,
    failover_service_tier: draft.failoverServiceTier,
...
export function fromStoredAgentDraft(
  ...
  return {
    ...
    lightweightModel: stored.lightweight_model ?? "",
    lightweightThinkingLevel: stored.lightweight_thinking_level ?? "",
    startLightweight: stored.start_lightweight ?? true,
    failoverRuntimeId: stored.failover_runtime_id ?? "",
    failoverModel: stored.failover_model ?? "",
    failoverThinkingLevel: stored.failover_thinking_level ?? "",
    failoverServiceTier: stored.failover_service_tier ?? "",
