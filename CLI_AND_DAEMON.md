...
The daemon launches Goose as `goose run -i - --output-format stream-json`, with the prompt on stdin. Resume is `--resume --session-id <id>` together. A bare `--resume` is never emitted. It writes the task brief to `AGENTS.md` and skills to `.agents/skills/`. `GOOSE_MODE=auto` is pinned on the process. There is no `MULTICA_GOOSE_MODEL` until Goose can list models without a signed-in provider. Multica-managed MCP is held until an isolated `--no-profile --with-extension` canary.
